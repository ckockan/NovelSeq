...
void itoa(int value, char* result) {
				...
				int quotient = value;
				do {
								*out = "0123456789abcdef"[ std::abs( quotient % 10) ];
								++out;
								quotient /= 10;
				} while ( quotient );
...
void mergeToType2(char* type1Name, char* type2Name)
{
				fstream type1(type1Name, fstream::in);
				fstream type2(type2Name, fstream::out);
	  //stream* cluster_fasta = NULL;
				...
				while(!type1.eof())
				{
						...
						if (!isF && !strcmp("F", temp_token[2]))
							{
												isF = !isF;
										//		cout << !isF << " " << name << temp_token[2] << endl;
												type2 << "END" << endl;
												type2 << clusterId << ((isF) ? " F" : " R") << endl;
...
int main(int argc, char** argv)
{
				if (argc != 3)
				{
								cout << "Error in inputs! Usage: <cluster_type_1> <cluster_type_2> "  << endl;
								return 0;
				}

//				cout << "IMAN NEW VERSION" << endl;
	...
	//			createReadNames(nameseqfile, fasta_file);
				...
				mergeToType2(type1Name, type2Name);
