...
int main(int argc, char *argv[])
{
   ...
   fprintf(stdout, "USAGE: <histogram> <cluster file> <maxNumReads> [<minClusterId> <maxClusterId>] \n\n");
   ...
   if (argc == 5)
      readAllClusters = true;
   else
   {
      clusterId_Min=atoi(argv[5]);
      clusterId_Max=atoi(argv[6]);
   }
   ...
   while(fscanf(fpOEAReads, "%i %c", &clusterId, &clusterOrient)!=EOF)
   {
      listEdgeEl=NULL;
      read_OEA_reads(fpOEAReads);

      if (readAllClusters || clusterId>clusterId_Min && clusterId<clusterId_Max)
      {
         createTheGraph();
         resultStr=greedyGraphTraversal(graph_weights_matrix,read_number);
         printf("%i %c\n", clusterId, clusterOrient);
         printf("%s\n", resultStr);
         for (int count=0; count<read_number; count++)
         {
