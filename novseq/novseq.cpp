...
int main(int argc, char** argv)
{
				if (argc < 8)
				{
								printf("Not enough arguments passed. Usage: novseq <dir> <dataset> <satRemChrs> <chroDir> <histogram> <maxNumOfReadsEachCluster> <maxInsertSize>  <minSupportLeft> [minSupportRight]\r\n");
								return -1;
				}
				...
				sprintf(command, "cut -f1,8 %s.parsed > %s.Name_Seq", dataset, dataset);
				runCommand(command);

				sprintf(command, "./unique %s.Name > %s.Name.UniqueTemp", dataset, dataset);
				runCommand(command);

				sprintf(command, "sort -u %s.Name.UniqueTemp > %s.Name.Unique", dataset, dataset);
				runCommand(command);
				...
				sprintf(command, "./type1to2convertor %s.clusters %s.type2 %scluster_reads/", dataset, dataset, dir);
				runCommand(command);


//				sprintf(command, "/net/eichler/vol5/home/imanh/abyss-1.2.7/ABYSS/ABYSS %s.clusters %s.all 35000011 > %s.type2", dataset, dataset, dataset, dataset);
//				runCommand(command);
